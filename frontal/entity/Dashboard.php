...
class Dashboard {
    ...
    */
    ...
    public function getCheckedUrl($from_time="", $to_time="") {
        ...
        $a="";
        if(isset($get_array["_a"])) $a="&_a=".$get_array["_a"];
        elseif(isset($get_array["amp;_a"])) $a="&_a=".$get_array["amp;_a"];
	...
	if(isset($g)) {
		$g = preg_replace(
				...
				array("refreshInterval:(pause:!t,value:0)","time:(from:'".$from_time."',to:'".$to_time."')"),
				$g);
		if (strpos( $g , "refreshInterval:(" ) === false ) {
			$g .= "refreshInterval:(pause:!t,value:0)";
		}
		...
		$g = "(refreshInterval:(pause:!t,value:0),time:(from:'".$from_time."',to:'".$to_time."'))";
	}

        //preg_match('/timestamp:\[(.*) TO (.*)\]/', $a, $matches);

        if(preg_match('/timestamp:\[(.*) TO (.*)\]/', $a, $matches)==1) {
            $g="(refreshInterval:(pause:!t,value:0),time:(from:".$matches[1].",to:".$matches[2]."))";
        }


        $new_query = "embed=true&_g=".$g.$a;
