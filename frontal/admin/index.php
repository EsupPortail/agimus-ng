<?php
    ...
    if($user->isAdmin()) {
      ...
      } elseif(''.$root_uri.'/admin/index.php/exports'==$uri ) {
        require_once $index_path.'/admin/views/exportViews.php';       
        if(isset($_GET['action']) && $_GET['action']=="view" && isset($_GET['id'])) view_export_action($_GET['id']);
        elseif(isset($_GET['action']) && $_GET['action']=="delete" && isset($_GET['id'])) delete_export_action($_GET['id']);
        elseif(isset($_GET['action']) && $_GET['action']=="reactive" && isset($_GET['id'])) reactive_export_action($_GET['id']);
        elseif(isset($_GET['action']) && $_GET['action']=="desactive" && isset($_GET['id'])) desactive_export_action($_GET['id']);
        else list_all_export_action();

      } elseif(''.$root_uri.'/admin/index.php/dash_reorder'==$uri ) {        
        update_dashreorder();        
